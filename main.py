...
def create_cell(e):
    ...
    #cells.append((canvas.create_rectangle(x,y,x+vs,y+vs, fill = "black")))
    cells[tx,ty] = 1
    print(getnei(tx,ty))


def redraw_cells():
    #prechadzam cells, ak tam je 1, vykreslim bunku na prislusnej pozicii(treba dopocitat) a rozmeroch
...
def lifegame():
    if button1.config('text')[-1] == 'No cells':
        button1.config(text='New cells')
        loopFunction()
    else:
        button1.config(text='No cells')
